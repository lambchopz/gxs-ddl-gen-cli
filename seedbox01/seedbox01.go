// Package seedbox01 handles all link generation for data.project-gxs.com and torrents.project-gxs.com
package seedbox01
...
func GenerateDDL(filter string, shorten bool, video bool, torrent bool, html string, secret []byte) {
	fmt.Println()
	fmt.Println("---------- Seedbox-01 ----------")

	video_files_url := "www/data.project-gxs.com/public_html"
	torrent_files_url := "www/torrents.project-gxs.com"
...
func establishSSHConnection(url_path string, secret []byte) string {
	...
	client, err := ssh.Dial("tcp", "atlas.feralhosting.com:22", config)
	if err != nil {
...
func parseVideoOutput(filter string, shorten bool, output string) {
	...
	for scanner.Scan() {
		...
		if strings.Contains(strings.ToLower(line), strings.ToLower(filter)) {
			if strings.HasPrefix(line, "www/data.project-gxs.com/public_html/") {
				fmt.Println()

				title = strings.Replace(line, "www/data.project-gxs.com/public_html/", "", 1)
				...
				if shorten {
					shortened_link := getShorteLink("http://data.project-gxs.com/" + title + line)
					
					fmt.Println(line)
					fmt.Println(shortened_link)
				} else {
					fmt.Println("http://data.project-gxs.com/" + title + line)
				}
			}
		}
	}
	...
	for scanner.Scan() {
		...
		if strings.Contains(strings.ToLower(line), strings.ToLower(filter)) {
			if strings.Contains(line, ".torrent") && strings.Contains(line, "[project-gxs]") {
				line = line[strings.Index(line, "[project-gxs]"):]
				if shorten {
					shortened_link := getShorteLink("http://torrents.project-gxs.com/" + line)
					
					fmt.Println(line)
					fmt.Println(shortened_link)
				} else {
					fmt.Println("http://torrents.project-gxs.com/" + line)
				}
			}
		}
	}
		...
		if strings.Contains(strings.ToLower(line), strings.ToLower(filter)) {
			...
			if strings.HasPrefix(line, "www/data.project-gxs.com/public_html/") {
				...
				if html == "batch" && title != "" && total_size != "" {
					...
					if shorten {
						shortened_link := getShorteLink("http://torrents.project-gxs.com/" + title2 + ".torrent")
						
						fmt.Print(shortened_link)
					} else {
						fmt.Print("http://torrents.project-gxs.com/" + title2 + ".torrent")
					}
				...
				if shorten {
					shortened_link := getShorteLink("http://data.project-gxs.com/" + title2 + line)
						
					fmt.Print(shortened_link)
				} else {
					fmt.Print("http://data.project-gxs.com/" + title2 + line)
				}
				...
				if html == "non-batch" {
					...
					if shorten {
						shortened_link := getShorteLink("http://torrents.project-gxs.com/" + line + ".torrent")
						
						fmt.Print(shortened_link)
					} else {
						fmt.Print(`http://torrents.project-gxs.com/` + line + ".torrent")
					}
